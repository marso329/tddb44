...
sym_index ast_elsif::generate_quads(quad_list &q) {
	sym_index adr = condition->generate_quads(q);
...
sym_index ast_integer::generate_quads(quad_list &q) {
	sym_index adr = sym_tab->gen_temp_var(type);
	q += new quadruple(q_iload, value, NULL_SYM, adr);
	return adr;
}

sym_index ast_real::generate_quads(quad_list &q) {
	sym_index adr = sym_tab->gen_temp_var(type);
	q += new quadruple(q_rload, value, NULL_SYM, adr);
	return adr;
}
...
sym_index do_unary(quad_list &q, ast_expression* expr, quad_op_type quad_op) {
	sym_index exp = expr->generate_quads(q);
	sym_index adr = sym_tab->gen_temp_var(expr->type);
	q += new quadruple(quad_op, exp, NULL_SYM, adr);
	return adr;
}

sym_index ast_not::generate_quads(quad_list &q) {
	sym_index exp = expr->generate_quads(q);
	sym_index adr = sym_tab->gen_temp_var(integer_type);
	q += new quadruple(q_inot, exp, NULL_SYM, adr);
	return adr;
}
...
sym_index ast_uminus::generate_quads(quad_list &q) {
	...
	if (expr->type == integer_type) {
		q += new quadruple(q_iuminus, exp, NULL_SYM, adr);
	} else {
		q += new quadruple(q_ruminus, exp, NULL_SYM, adr);
	}
	return adr;
}
...
sym_index ast_cast::generate_quads(quad_list &q) {
	sym_index exp = expr->generate_quads(q);
	sym_index adr = sym_tab->gen_temp_var(real_type);
	q += new quadruple(q_itor, exp, NULL_SYM, adr);
	return adr;
}

sym_index binop(quad_list &q, ast_binaryoperation* op, quad_op_type iop,
		quad_op_type rop) {
	sym_index left = op->left->generate_quads(q);
	sym_index right = op->right->generate_quads(q);
	sym_index left_type = sym_tab->get_symbol_type(left);
	sym_index adr = NULL_SYM;

	adr = sym_tab->gen_temp_var(left_type);
	if (left_type == integer_type) {
		q += new quadruple(iop, left, right, adr);
	} else if (left_type == real_type) {
		q += new quadruple(rop, left, right, adr);
	}
	return adr;
}

sym_index ast_add::generate_quads(quad_list &q) {
	return binop(q, this, q_iplus, q_rplus);
}

sym_index ast_sub::generate_quads(quad_list &q) {
	return binop(q, this, q_iminus, q_rminus);
}

sym_index ast_mult::generate_quads(quad_list &q) {
	return binop(q, this, q_imult, q_rmult);
}

sym_index ast_divide::generate_quads(quad_list &q) {
	return binop(q, this, q_nop, q_rdivide);
}

sym_index ast_idiv::generate_quads(quad_list &q) {
	return binop(q, this, q_idivide, q_nop);
}

sym_index ast_mod::generate_quads(quad_list &q) {
	return binop(q, this, q_imod, q_nop);
}

sym_index ast_or::generate_quads(quad_list &q) {
	return binop(q, this, q_ior, q_nop);
}

sym_index ast_and::generate_quads(quad_list &q) {
	return binop(q, this, q_iand, q_nop);
}

sym_index binrel(quad_list &q, ast_binaryrelation* rel, quad_op_type iop,
		quad_op_type rop) {
	sym_index left = rel->left->generate_quads(q);
	sym_index right = rel->right->generate_quads(q);
	sym_index left_type = sym_tab->get_symbol_type(left);
	sym_index adr = sym_tab->gen_temp_var(left_type);
	if (left_type == integer_type) {
		q += new quadruple(iop, left, right, adr);
	} else {
		q += new quadruple(rop, left, right, adr);
...
void ast_indexed::generate_assignment(quad_list &q, sym_index rhs) {
    ...
    sym_index adr = sym_tab->gen_temp_var(integer_type);
    q += new quadruple(q_lindex, id->sym_p, pos, adr);

    if (id->type == integer_type) {
        q += new quadruple(q_istore, rhs, NULL_SYM, adr);
    } else if (id->type == real_type) {
        q += new quadruple(q_rstore, rhs, NULL_SYM, adr);
    } else {
...
void ast_expr_list::generate_parameter_list(quad_list &q,
	...
	sym_index adr = last_expr->generate_quads(q);
	q += new quadruple(q_param, adr, NULL_SYM, NULL_SYM);
	*nr_params += 1;
	if (preceding != NULL) {
...
sym_index ast_procedurecall::generate_quads(quad_list &q) {
	...
	parameter_symbol* last_param =
			sym_tab->get_symbol(id->sym_p)->get_procedure_symbol()->last_parameter;
...
void ast_elsif::generate_quads_and_jump(quad_list &q, int label) {
	sym_index last = sym_tab->get_next_label();
	sym_index res = condition->generate_quads(q);
	q += new quadruple(q_jmpf, last, res, NULL_SYM);
	if (body != NULL) {
		body->generate_quads(q);
	}
	q += new quadruple(q_jmp, label, NULL_SYM, NULL_SYM);
	q += new quadruple(q_labl, last, NULL_SYM, NULL_SYM);
}
