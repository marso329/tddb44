...
sym_index ast_indexed::type_check() {
	if (index->type_check() != integer_type)
		type_error(index->pos) << "Index can only be integer type" << endl;
...
sym_index semantic::check_binop1(ast_binaryoperation *node) {
	...
	if (!(left_type == real_type || left_type == integer_type)
		...
		type_error(node->pos) << "None compatible types " << left_type << " "
				<< right_type << endl;
	}
...
sym_index semantic::check_binop2(ast_binaryoperation *node, string s) {
	if (node->left->type_check() != integer_type
		...
		type_error(node->pos) << "Only integer operation allowed with:" << s
				<< endl;
...
/* Convenience method for all binary relations, since they're all typechecked
...
sym_index ast_if::type_check() {
	if (condition->type_check() != integer_type)
		type_error(condition->pos)
				<< "Condition statement must return integer type" << endl;
...
sym_index ast_functioncall::type_check() {
	type_checker->check_parameters(id, parameter_list);
	return sym_tab->get_symbol(id->sym_p)->get_function_symbol()->type;
}
...
sym_index ast_not::type_check() {
	  if(expr->type_check() != integer_type){
	    type_error(expr->pos) << "Not requires integer type" << endl;
	    return void_type;
	  }
...
sym_index ast_elsif::type_check() {
	  if(condition->type_check() != integer_type)
	    type_error(condition->pos) << "elseif requires integer type" << endl;
